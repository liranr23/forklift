...
func convert(srcVolPath, dstVolPath, srcFormat, dstFormat string) error {
...
func ovaExport(vm *cnv.VirtualMachine) (err error) {
	var diskPopulatedMap map[string]string
	for i, path := range vm.Spec.Template.Spec.Volumes {
		size, err := qemuimgMeasure(path)
		if err != nil {
			return err
		}
		diskPopulatedMap[pvcNames[i]] = size
	}
	ova.WriteOvf(vm, diskPopulatedMap)
}
